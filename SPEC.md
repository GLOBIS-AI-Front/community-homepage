...
| 項目              | 採用                                                      | 根拠                                                                                                                                     |
| ----------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| フレームワーク    | **Next.js 16 (App Router)**                               | RSC による高速初期描画、SSG 中心。静的エクスポート（`output: 'export'`）で Cloudflare Pages にそのままデプロイ可能                       |
...
| デプロイ          | **Cloudflare Pages（静的エクスポート）**                  | `next build` で生成される `out/` を Pages に配信。無料枠が広く、グローバルエッジ配信に標準対応。静的サイトのため運用コスト最小           |
| CDN / 配信        | **Cloudflare Edge Network**（Pages 配下で自動構成）       | グローバルエッジ配信、`Cache-Control` ヘッダ制御、HTTP/3 / Brotli 等を標準で利用                                                         |
| CI/CD             | **Cloudflare Pages GitHub Integration**                   | push / PR に応じて自動ビルド・デプロイ。プレビュー URL 自動発行、追加シークレット管理不要                                                |
...
| 2026-04-24 | ホスティングを Vercel から Cloudflare Pages（静的エクスポート）に変更 |
